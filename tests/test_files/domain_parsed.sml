signature ABS_DOMAIN = sig type t val layout : t -> Layout.t val top : t val bottom : t val tuple : t list -> t val func : ( t -> t ) -> t val isBottom : t -> bool val isTop : t -> bool val isTuple : t -> ( t list ) option val isFunc : t -> ( t -> t ) option val glb : t list -> t val lub : t list -> t  end structure Pointed : ABS_DOMAIN = struct 
 ...
 | layout ( Tuple l ) = LayoutUtils.sequence ( "<" , ">" , "," ) ( List.map ( l , layout ) )   end 
