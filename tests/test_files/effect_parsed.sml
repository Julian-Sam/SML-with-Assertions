signature EFFECT = sig eqtype set eqtype effect val Total : set val PAny : set val ReadOnly : set val Heap : set val Init : set val HeapInit : set val Control : set val Any : set val Partial : effect val Io : effect val HeapGen : effect val HeapRead : effect val HeapWrite : effect val Throws : effect val Returns : effect val Fails : effect val InitGen : effect val InitRead : effect val InitWrite : effect val PartialS : set val IoS : set val HeapGenS : set val HeapReadS : set val HeapWriteS : set val ThrowsS : set val ReturnsS : set val FailsS : set val InitGenS : set val InitReadS : set val InitWriteS : set val effects : effect list val isEmpty : set -> bool val contains : set * effect -> bool val subset : set * set -> bool val fromList : effect list -> set val add : set * effect -> set val remove : set * effect -> set val single : effect -> set val union : set * set -> set val unionL : set list -> set val intersection : set * set -> set val difference : set * set -> set val effectToChar : effect -> char val charToSet : char -> set option val layout : set -> Layout.t val compare : set * set -> order  end ; structure Effect :> EFFECT = struct structure WordN = Word32 
 ...
 fun effectToChar f = if f = Partial then #"P" else if f = Io then #"I" else if f = HeapGen then #"N" else if f = HeapRead then #"R" else if f = HeapWrite then #"W" else if f = Throws then #"T" else if f = Returns then #"X" else if f = Fails then #"F" else if f = InitGen then #"g" else if f = InitRead then #"r" else if f = InitWrite then #"w" else Fail.fail ( "Effect" , "effectToChar" , "Impossible effect" )  structure CD = DictF ( struct 

 type t = char 

 val compare = Char.compare  end ) 

 val charToSetMap : set CD.t = CD.fromList ( List.map ( effects , fn e => ( effectToChar e , single e ) ) @ [ ( #"A" , PAny ) , ( #"a" , Any ) ] ) 

 fun charToSet c = CD.lookup ( charToSetMap , c )  local structure L = Layout structure LU = LayoutUtils  in 
 ...
 val l = LU.brace ( L.seq ( l1 :: ls ) )  in l end   end 

 val compare = WordN.compare  end ; 
