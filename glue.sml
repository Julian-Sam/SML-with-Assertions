...
fun readFromLists (ws: int list, tb: int list, nl: int list, len: int, final: ws_type list) =
	case (ws, tb, nl) of
 ...
 *)
  ...
  fun parse (filename) = 
    ...
    val a: (int list * int list * int list) ref = ref ([], [], [])
    val dummyEOF = SampleLrVals.Tokens.EOF(a, 0, 0)
    fun loop lexer =
